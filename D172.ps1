...
function FinnNaboer ($cube) {
    ...
    [int]$x, [int]$y, [int]$z, [int]$w = $cube.Split("_")
    $neighbors = @()

    for ($d_x=-1; $d_x -le 1; $d_x++) {
        for ($d_y=-1; $d_y -le 1; $d_y++) {
            for ($d_z=-1; $d_z -le 1; $d_z++) {
                for ($d_w=-1; $d_w -le 1; $d_w++) {
                    if (!(($d_x -eq 0) -and ($d_y -eq 0) -and ($d_z -eq 0) -and ($d_w -eq 0))) {
                        $neighbors += "$($x+$d_x)" + "_" + "$($y+$d_y)" + "_" + "$($z+$d_z)" + "_" + "$($w+$d_w)"
                    }
                }
            }
        }
...
function SokOmraade ($conway_space) {
    ...
    foreach ($koor in $conway_space.Keys) {
        [int]$x, [int]$y, [int]$z, [int]$w = $koor.Split("_")
    ...
    for ($d_x=$x_min; $d_x -le $x_max; $d_x++) {
        for ($d_y=$y_min; $d_y -le $y_max; $d_y++) {
            for ($d_z=$z_min; $d_z -le $z_max; $d_z++) {
                for ($d_w=$w_min; $d_w -le $w_max; $d_w++) {
                    $search_space += "$d_x" + "_" + "$d_y" + "_" + "$d_z" + "_" + "$d_w"
                }
...
Write-Host "Dette tok: $($stopwatch.Elapsed.TotalSeconds) sek" -f Cyan
